...
export const register = catchAsyncError(async (req, res, next) => {
  try {
    ...
    if (registerationAttemptsByUser.length > 3) {
      return next(
        new ErrorHandler(
          "Maximum registration attempts (3) exceeded. Please try again after an hour.",
          400
        )
      );
    }
...
// Function to send verification code via email or SMS
  ...
  try {
    ...
    // Send verification code via SMS
      ...
      const twilioResult = await client.messages.create({
        body: `${name} wrote the following: "${verificationCodeWithSpace}"`,
